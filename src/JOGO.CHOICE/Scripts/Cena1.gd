...
#funcao para apagar os circles do mapa quando  o personagem coleta eles, utiliza da variavel 'numberArtifacts'
func circles():
...
func morte():
	if Classglobal.lives <= 0:
		...
		Classglobal.coins = 0
#funcao para sinalizar ao player para apertar E para iniciar o quiz
func quiz():
	if Classglobal.quiz == true:
		$Player/Control2.visible = false
#funcao que realiza o checkpoint com base no numero de artefatos em maos quando o personagem morreu
func checkpoint():
	if Classglobal.numberArtifacts >= 3:
